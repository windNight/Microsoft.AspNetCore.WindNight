...
namespace Microsoft.AspNetCore.GRpc.HttpApi
{
        ...
        where TResponse : class
    {
        ...
        public async Task HandleCallAsync(HttpContext httpContext)
        {
            ...
            {
                responseMessage =
                    await _unaryMethodInvoker.Invoke(httpContext, serverCallContext, (TRequest) requestMessage);
            }
            catch (Exception ex)
            {
        ...
        private async Task SendErrorResponse(HttpResponse response, Encoding encoding, string message,
            StatusCode statusCode)
        {
            var e = new Error
            {
        ...
        {
            await using (var writer = new HttpResponseStreamWriter(response.Body, encoding))
            {
        ...
        private bool CanBindQueryStringVariable(string variable)
        {
            if (_bodyDescriptor != null)
            {
                if (_bodyFieldDescriptors == null || _bodyFieldDescriptors.Count == 0) return false;

                if (variable == _bodyFieldDescriptorsPath) return false;

                if (variable.StartsWith(_bodyFieldDescriptorsPath!, StringComparison.Ordinal)) return false;
            }

            if (_routeParameterDescriptors.ContainsKey(variable)) return false;
